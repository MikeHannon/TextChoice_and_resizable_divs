...
function generate_outlines(){

  var myLength = $('#container').children().length;
  for (var i =0; i < myLength; i++){
    var k = random_color();
    $('#container').children().eq(i).css("border", "1px dotted " + k);
    $('#container').children().eq(i).draggable({stop:function(e){
...
function are_mobile(){
  ...
  var myLength = $('#container').children().length;
  for (var i =0; i < myLength; i++){
    $('#container').children().eq(i).resizable({stop:function(e){
      var rect = this.getBoundingClientRect();
      console.log(rect.top, rect.right, rect.bottom, rect.left);
      console.log(rect.height, rect.width);
      }
...
function apply_to_all(myVar, myFunction, my){
